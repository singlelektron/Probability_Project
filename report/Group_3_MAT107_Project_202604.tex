...
\vfill\pagebreak
\section{Approximating the lower binomial tail}
...
\begin{proof}
Every unordered sample of 20 distinct balls is equally likely. There are \(\binom{308}{20}\) such samples. A sample containing exactly \(x\) white balls can be formed by choosing \(x\) of the 117 white balls and independently choosing \(20-x\) of the 191 black balls. Hence the number of favourable samples is \(\binom{117}{x}\binom{191}{20-x}\), which proves the probability formula. Summing the favourable counts over all possible \(x\) partitions every sample according to its number of white balls, so the probabilities sum to one.
