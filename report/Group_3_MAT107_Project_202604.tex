...
P(BBB)=q^3=0.238478.
\]
In the simulation, this event occurred 114 times, yielding an empirical frequency of 0.228000. Its absolute deviation was 0.010478 and its relative error was 4.3936\%. The larger percentage error, compared with the overall white proportion, is reasonable because this calculation uses only 500 three-draw sequences rather than 10,000 individual draws.
...
We next examine the lower tail, i.e.\ fewer than six white balls (\(X\leq5\)). Its exact binomial probability is
\[
P(X<6)=\sum_{x=0}^{5}\binom{20}{x}p^xq^{20-x}=0.167366.
\]
From the 500 experiments, this event occurred 84 times, giving an empirical frequency of 0.168000. Its relative error from the exact value was only 0.3787\%.

For the Poisson approximation, \(\lambda=np=7.597403\), so
\[
P(X<6)\approx \sum_{x=0}^{5}e^{-\lambda}\frac{\lambda^x}{x!}=0.230956.
\]
The Poisson value differs from the exact binomial probability by 0.063589, a relative error of 37.9941\%. The approximation is poor because \(p\approx0.38\) is not small, whereas the Poisson approximation is most reliable when the number of trials is large and the success probability is small.
...
For the without-replacement case, the sample mean across 500 experiments was 7.606000. Its absolute deviation from 7.597403 was 0.008597, or 0.1132\%. Table~\ref{tab:hyperpmf} compares the hypergeometric probabilities with the simulated relative frequencies.
...
=0.237049.
\]
In the simulation, this event occurred 123 times (empirical frequency 0.246000), producing an absolute deviation of 0.008951 and a relative error of 3.7760\%.
